...
def plot_quadrant_actual_gain(module_scores: pd.DataFrame, training_log: pd.DataFrame, output_dir: str, future_eval_points: int):
    ...
    # Step-macro average: first aggregate within each scoring step, then average over steps.
    # This avoids a single bad global eval window dominating quadrants with larger module counts.
    step_summary = (
        ms.groupby(["step", "quadrant"], as_index=False)
        .agg(
            avg_I=("ema_I", "mean"),
            avg_P=("ema_P", "mean"),
    ...
    summary = (
        step_summary.groupby("quadrant", as_index=False)
        .agg(
            avg_I=("avg_I", "mean"),
            avg_P=("avg_P", "mean"),
...
def plot_goodput_curve(module_scores: pd.DataFrame, output_dir: str):
    """图2: Goodput 曲线 (X=step, Y=mean goodput).

    Validates whether IPD-GP-LoRA yields higher learning progress per budget.
    """
    ...
    for step in steps:
        ...
        if df.empty:
            continue
        valid_steps.append(step)
        mean_ema_G.append(float(df["ema_G"].mean()))
        if "current_G" in df.columns:
            mean_cur_G.append(float(df["current_G"].mean()))
    if not valid_steps:
        return
    plt.figure(figsize=(8, 5))
    plt.plot(valid_steps, mean_ema_G, marker="o", label="mean ema_G")
    if mean_cur_G:
        plt.plot(valid_steps, mean_cur_G, marker="s", alpha=0.6, label="mean current_G")
    plt.axhline(0.0, linestyle="--", color="black", linewidth=1)
    plt.xlabel("step")
    plt.ylabel("Module Learning Goodput")
    plt.title("Mean Module Learning Goodput over Training")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "goodput_curve.png"), dpi=180)
    plt.close()


def plot_global_goodput_curve(training_log: pd.DataFrame, output_dir: str):
    """全局 Goodput / Time-Goodput / FLOP-Goodput 曲线 (来自 training_log)."""
    ...
    panels = [
        ("rank_step", "ΔValLoss / Rank-Step"),
        ("per_second", "ΔValLoss / Second"),
        ("per_flop", "ΔValLoss / FLOPs(proxy)"),
    ]
    for ax, (col, title) in zip(axes, panels):
        ...
        ax.set_xlabel("step")
        ax.set_title(title)
    fig.suptitle("Global Goodput (rank-step / time / flop)")
    fig.tight_layout()
    fig.savefig(os.path.join(output_dir, "global_goodput_curve.png"), dpi=180)
    plt.close(fig)


def plot_ipg_correlation(module_scores: pd.DataFrame, output_dir: str):
    """图3: I/P/G 相关性. Spearman(I,G), Spearman(P,G), Spearman(I*P,G).

    Expected: corr(I,G) low, corr(P,G) medium, corr(I*P,G) highest.
    """
    if module_scores.empty or "ema_G" not in module_scores.columns:
        return
    steps = sorted(module_scores["step"].unique().tolist())
    s_ig, s_pg, s_ipg, valid_steps = [], [], [], []
    for step in steps:
        df = module_scores[module_scores["step"] == step]
        if len(df) < 3:
            continue
        I = df["ema_I"].to_numpy(dtype=np.float64)
        P = df["ema_P"].to_numpy(dtype=np.float64)
        G = df["ema_G"].to_numpy(dtype=np.float64)
        if np.std(G) <= 0:
            continue
        IP = I * P
        valid_steps.append(step)
        s_ig.append(spearmanr(I, G).correlation if np.std(I) > 0 else 0.0)
        s_pg.append(spearmanr(P, G).correlation if np.std(P) > 0 else 0.0)
        s_ipg.append(spearmanr(IP, G).correlation if np.std(IP) > 0 else 0.0)
    if not valid_steps:
        return

    plt.figure(figsize=(8, 5))
    plt.plot(valid_steps, s_ig, marker="o", label="Spearman(I, G)")
    plt.plot(valid_steps, s_pg, marker="s", label="Spearman(P, G)")
    plt.plot(valid_steps, s_ipg, marker="^", label="Spearman(I×P, G)")
    plt.axhline(0.0, linestyle="--", color="black", linewidth=1)
    plt.xlabel("step")
    plt.ylabel("Spearman correlation")
    plt.title("I/P/G Rank Correlation over Training")
    plt.ylim(-1.05, 1.05)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "ipg_correlation.png"), dpi=180)
    plt.close()

    # Overall (pooled over all scoring steps) correlations saved to CSV.
    df_all = module_scores.dropna(subset=["ema_I", "ema_P", "ema_G"]).copy()
    if len(df_all) >= 3 and np.std(df_all["ema_G"].to_numpy()) > 0:
        I = df_all["ema_I"].to_numpy(dtype=np.float64)
        P = df_all["ema_P"].to_numpy(dtype=np.float64)
        G = df_all["ema_G"].to_numpy(dtype=np.float64)
        overall = pd.DataFrame(
            [
                {
                    "spearman_I_G": float(spearmanr(I, G).correlation),
                    "spearman_P_G": float(spearmanr(P, G).correlation),
                    "spearman_IxP_G": float(spearmanr(I * P, G).correlation),
                    "mean_step_spearman_I_G": float(np.nanmean(s_ig)),
                    "mean_step_spearman_P_G": float(np.nanmean(s_pg)),
                    "mean_step_spearman_IxP_G": float(np.nanmean(s_ipg)),
                }
            ]
        )
        overall.to_csv(os.path.join(output_dir, "ipg_correlation_summary.csv"), index=False)


def plot_ipg_quadrant(module_scores: pd.DataFrame, output_dir: str):
    """图4: I-P-G 四象限图. X=Importance, Y=Plasticity, Color=Goodput."""
    if module_scores.empty or "ema_G" not in module_scores.columns:
        return
    last_step = int(module_scores["step"].max())
    df = module_scores[module_scores["step"] == last_step].copy()
    if df.empty:
        return
    x = df["ema_I"].to_numpy(dtype=np.float64)
    y = df["ema_P"].to_numpy(dtype=np.float64)
    c = df["ema_G"].to_numpy(dtype=np.float64)
    plt.figure(figsize=(8.5, 6.5))
    sc = plt.scatter(x, y, c=c, cmap="viridis", s=90, alpha=0.9, edgecolors="k", linewidths=0.4)
    plt.colorbar(sc, label="ema_G (goodput)")
    plt.axvline(float(np.median(x)), linestyle="--", color="gray", linewidth=1)
    plt.axhline(float(np.median(y)), linestyle="--", color="gray", linewidth=1)
    if {"layer_index", "projection_type"}.issubset(df.columns):
        for _, row in df.iterrows():
            label = f"{int(row['layer_index'])}-{str(row['projection_type']).replace('_proj','')}"
            plt.text(row["ema_I"], row["ema_P"], label, fontsize=6, alpha=0.7)
    plt.xlabel("Importance (ema_I)")
    plt.ylabel("Plasticity (ema_P)")
    plt.title(f"I-P-G Quadrant (color = Goodput) at step {last_step}")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "ipg_quadrant.png"), dpi=180)
    plt.close()


def compute_efficiency_metrics(training_log: pd.DataFrame, output_dir: str, threshold_ratio: float = 0.9):
    """收敛效率汇总: AULC / Steps-to-Threshold / Accuracy@Fixed-Steps / 全局 Goodput 均值."""
    if training_log.empty:
        return
    eval_df = training_log.dropna(subset=["eval_accuracy"]).copy()
    if "train_loss" in eval_df.columns:
        eval_df = eval_df[eval_df["train_loss"].isna()].copy()
    eval_df = eval_df.groupby("step", as_index=False).last().sort_values("step")
    if eval_df.empty:
        return
    ...
    # AULC: normalized area under the accuracy-vs-step learning curve.
    _trapz = getattr(np, "trapezoid", np.trapz)
    if len(steps) >= 2:
        span = float(steps[-1] - steps[0])
        aulc = float(_trapz(acc, steps) / span) if span > 0 else final_acc
    else:
        aulc = final_acc

    # Steps-to-Threshold: first step reaching threshold_ratio * best_acc.
    target = threshold_ratio * best_acc
    hit = np.where(acc >= target)[0]
    steps_to_threshold = int(steps[hit[0]]) if len(hit) > 0 else None

    # Accuracy @ fixed step fractions of the schedule.
    acc_at_fixed = {}
    max_step = float(steps[-1])
    for frac in (0.25, 0.5, 0.75, 1.0):
        target_step = frac * max_step
        idx = int(np.searchsorted(steps, target_step, side="right") - 1)
        idx = max(0, min(idx, len(acc) - 1))
        acc_at_fixed[f"acc@{int(frac*100)}%"] = float(acc[idx])

    # Mean global goodput across windows.
    gp_means = {}
    if "global_goodput" in training_log.columns:
        ...
        for _, r in training_log.iterrows():
            gp = r.get("global_goodput")
            if isinstance(gp, dict) and gp:
                rs.append(float(gp.get("goodput/rank_step", np.nan)))
                ps.append(float(gp.get("goodput/per_second", np.nan)))
                fs.append(float(gp.get("goodput/per_flop", np.nan)))
        if rs:
            gp_means = {
                "mean_goodput_rank_step": float(np.nanmean(rs)),
                "mean_goodput_per_second": float(np.nanmean(ps)),
                "mean_goodput_per_flop": float(np.nanmean(fs)),
            }
    ...
    with open(os.path.join(output_dir, "efficiency_metrics.json"), "w", encoding="utf-8") as f:
...
def main():
    ...
    # ----- Module Learning Goodput visualizations -----
    plot_goodput_curve(module_scores, output_dir)
    plot_global_goodput_curve(training_log, output_dir)
    plot_ipg_correlation(module_scores, output_dir)
    plot_ipg_quadrant(module_scores, output_dir)
    compute_efficiency_metrics(training_log, output_dir, threshold_ratio=0.9)
