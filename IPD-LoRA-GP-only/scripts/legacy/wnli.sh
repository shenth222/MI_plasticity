...
# GP-only dynamic-rank LoRA example runs for GLUE WNLI.
#
# Usage:
#   bash wnli.sh probing
#   bash wnli.sh proxy
#
# Optional overrides:
#   CUDA_VISIBLE_DEVICES=1 MODEL_PATH=/path DATA_ROOT=/path bash wnli.sh probing
...
if [[ "${MODE}" == "probing" ]]; then
  python train_ipd_lora.py "${COMMON_ARGS[@]}" \
    --output_dir "outputs/gp_only_lora_wnli_probing_${RUN_TAG}" \
    --goodput_method probing \
    --goodput_every_n_scoring 2 \
    --beta_G 0.9 \
    --goodput_probe_max_batches 6 \
    --goodput_min_rank 1 \
    --wandb_run_name "gp-only-lora-wnli-probing-${RUN_TAG}"
elif [[ "${MODE}" == "proxy" ]]; then
  python train_ipd_lora.py "${COMMON_ARGS[@]}" \
    --output_dir "outputs/gp_only_lora_wnli_proxy_${RUN_TAG}" \
    --goodput_method proxy \
    --goodput_every_n_scoring 1 \
    --beta_G 0.9 \
    --goodput_min_rank 1 \
    --wandb_run_name "gp-only-lora-wnli-proxy-${RUN_TAG}"
else
