...
# GP-only dynamic-rank LoRA example runs for GLUE MNLI.
#
# Usage:
#   bash mnli.sh probing
#   bash mnli.sh proxy
#
# Optional overrides:
#   CUDA_VISIBLE_DEVICES=1 MODEL_PATH=/path DATA_ROOT=/path bash mnli.sh probing
...
if [[ "${MODE}" == "probing" ]]; then
  python train_ipd_lora.py "${COMMON_ARGS[@]}" \
    --output_dir "outputs/gp_only_lora_mnli_probing_${RUN_TAG}" \
    --goodput_method probing \
    --goodput_every_n_scoring 2 \
    --beta_G 0.9 \
    --goodput_probe_max_batches 6 \
    --goodput_min_rank 1 \
    --wandb_run_name "gp-only-lora-mnli-probing-${RUN_TAG}"
elif [[ "${MODE}" == "proxy" ]]; then
  python train_ipd_lora.py "${COMMON_ARGS[@]}" \
    --output_dir "outputs/gp_only_lora_mnli_proxy_${RUN_TAG}" \
    --goodput_method proxy \
    --goodput_every_n_scoring 1 \
    --beta_G 0.9 \
    --goodput_min_rank 1 \
    --wandb_run_name "gp-only-lora-mnli-proxy-${RUN_TAG}"
else
